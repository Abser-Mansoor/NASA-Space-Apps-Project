...
field_id = ['T2M', 'SLP', 'PS', 'U10M', 'V10M', 'QV2M'] # ID of field in MERRA-2 - find ID here: https://gmao.gsfc.nasa.gov/pubs/docs/Bosilovich785.pdf 
field_name = ['2m air temperature', 'sea level pressure', 'surface pressure', '10m zonal wind', '10m meridional wind', '2m specific humidity'] # Name of field to be stored with downloaded data (can use any name you like)
database_name = 'M2T1NXASM' # Name of database in which field is stored, can be looked up by ID here: https://gmao.gsfc.nasa.gov/pubs/docs/Bosilovich785.pdf 
database_id = 'tavg1_2d_asm_Nx' # ID of database database in which field is stored, also can be looked up by ID here: https://gmao.gsfc.nasa.gov/pubs/docs/Bosilovich785.pdf 
locs = [('karachi', 24.8607, 67.0011)]
conversion_functions = [lambda x: x - 273.15, lambda x: x, lambda x: x, lambda x: x, lambda x: x] # Unit conversion functions for each field
aggregators = ['mean', 'mean', 'mean', 'mean', 'mean'] # Method by which data will be aggregated over days and weeks. Can be "sum", "mean", "min", or "max"
